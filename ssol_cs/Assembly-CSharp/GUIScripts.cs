...
public class GUIScripts : MonoBehaviour
{
	...
	private void OnGUI()
	{
		if (!this.state.MovementFrozen)
		{
			...
			int num6 = 10;
			RectOffset padding = new RectOffset(num6, num6, num6, 0);
			float num7 = this.fontSizes.y / 1.75f;
			GUI.skin.box.padding = padding;
			Vector2 vector2 = GUI.skin.box.CalcSize(new GUIContent("test"));
			float num8 = (float)Screen.height - this.barSize.y;
			float num9 = (float)GameState.wrSplits.Length * vector2.y;
			float num10;
			if (num9 > num8)
			{
				num10 = num8 / num9;
				...
				padding = new RectOffset(num6, num6, num6, 0);
			}
			else
			{
				num10 = 1f;
			...
			GUI.skin.box.fontSize = (int)num7;
			GUI.skin.box.font = this.myStyle.font;
			GUI.skin.label = new GUIStyle(this.myStyle);
			GUI.skin.label.fontSize = (int)num7;
			GUI.skin.label.padding = padding;
			GUI.color = new Color(GUI.color.r, GUI.color.g, GUI.color.b, 1f);
			Vector2 vector3 = GUI.skin.box.CalcSize(new GUIContent("test"));
			Vector2 vector4 = GUI.skin.label.CalcSize(new GUIContent("Time: 000.000"));
			float x = GUI.skin.label.CalcSize(new GUIContent("-000.000")).x;
			if (this.showTimer)
			{
				float num11 = 400f * num10;
				string text = this.state.TotalTimePlayer.ToString("Time: 000.000");
				string text2 = this.state.playerVelocity.ToString("Speed: 00.000");
				GUI.Box(new Rect(0f, 0f, vector4.x + 10f * num10, vector4.y), new GUIContent(text));
				GUI.Box(new Rect(0f, 0f + vector4.y, vector4.x + 10f * num10, vector4.y), new GUIContent(text2));
				GUI.skin.box.alignment = TextAnchor.MiddleRight;
				GUI.Box(new Rect((float)Screen.width - num11, 0f, num11, vector3.y), new GUIContent("Time"));
				GUI.skin.box.normal.background = null;
				GUI.Box(new Rect((float)Screen.width - x, 0f, vector3.x, vector3.y), new GUIContent("+/-"));
				GUI.skin.box.alignment = TextAnchor.MiddleLeft;
				for (int i = 0; i < GameState.splitOn.Length; i++)
				{
					GUI.skin.box.normal.background = this.bgTex[i % 2];
					double num12 = this.state.splits[i];
					GUI.Box(new Rect((float)Screen.width - num11, vector3.y * (float)(i + 1), num11, vector3.y), new GUIContent(GameState.splitNames[i]));
					if (num12 > 0.0)
					{
						GUI.skin.box.normal.background = null;
						string text3 = num12.ToString("F03");
						vector3 = GUI.skin.box.CalcSize(new GUIContent(text3));
						float num13 = (float)num12 - GameState.wrSplits[i];
						string text4 = ((num13 < 0f) ? "" : "+") + num13.ToString("F02");
						Vector2 vector5 = GUI.skin.box.CalcSize(new GUIContent(text4));
						GUI.Label(new Rect((float)Screen.width - vector3.x, vector3.y * (float)(i + 1), vector3.x, vector3.y), text3);
						Color color = GUI.color;
						GUI.color = ((num13 < 0f) ? Color.green : Color.red);
						GUI.Label(new Rect((float)Screen.width - vector3.x - x, vector3.y * (float)(i + 1), vector5.x, vector5.y), text4);
						GUI.color = color;
					}
				}
			}
			...
			if (this.showOrbs)
			{
				...
				Vector2 vector6 = GUI.skin.box.CalcSize(new GUIContent("00"));
				GUI.skin.box.padding = new RectOffset((int)(Math.Max(0f, GUIScripts.defaultOrbBoxSize - vector6.x) / 2f), 0, (int)((GUIScripts.defaultOrbBoxSize - vector6.y) / 2f), 0);
				GUI.BeginGroup(new Rect(10f, 100f, num14, 400f));
				this.DrawRecentOrbs();
				GUI.EndGroup();
				GUI.skin.box.padding = new RectOffset(0, 0, 0, 0);
				vector6 = GUI.skin.box.CalcSize(new GUIContent("@"));
				GUI.skin.box.padding = new RectOffset((int)((GUIScripts.defaultOrbBoxSize - vector6.x) / 2f), 0, (int)((GUIScripts.defaultOrbBoxSize - vector6.y) / 2f), 0);
				GUI.BeginGroup(new Rect(10f, 510f, num14, 400f));
				this.DrawOrbsTotal();
				GUI.EndGroup();
			}
			GUI.EndGroup();
			float num15 = this.keyDimensions + this.keyPadding;
			GUI.skin.label.fontSize = (int)(this.fontSizes.y * 1f);
			GUI.BeginGroup(new Rect((this.showOrbs ? num14 : 0f) + 20f, (float)Screen.height - this.barSize.y - 2f * this.keyDimensions - 2.5f * this.keyPadding, 3f * num15, 2f * num15));
			float num16 = this.keyPadding / 2f;
			GUI.Box(new Rect(0f, num15, num15, num15), this.bgTex[0]);
			GUI.Box(new Rect(num15, num15, num15, num15), this.bgTex[0]);
			GUI.Box(new Rect(num15 * 2f, num15, num15, num15), this.bgTex[0]);
			GUI.Box(new Rect(num15, 0f, num15, num15), this.bgTex[0]);
			GUI.BeginGroup(new Rect(num16, num16, 3f * num15 - this.keyPadding, 2f * num15 - this.keyPadding));
			GUI.Button(new Rect(0f, num15, this.keyDimensions, this.keyDimensions), "A", Input.GetKey(KeyCode.A) ? this.pressedKeyStyle : this.keyStyle);
			GUI.Button(new Rect(num15 * 1f, num15, this.keyDimensions, this.keyDimensions), "S", Input.GetKey(KeyCode.S) ? this.pressedKeyStyle : this.keyStyle);
			GUI.Button(new Rect(num15 * 2f, num15, this.keyDimensions, this.keyDimensions), "D", Input.GetKey(KeyCode.D) ? this.pressedKeyStyle : this.keyStyle);
			GUI.Button(new Rect(num15, 0f, this.keyDimensions, this.keyDimensions), "W", Input.GetKey(KeyCode.W) ? this.pressedKeyStyle : this.keyStyle);
			GUI.EndGroup();
			GUI.EndGroup();
		}
		else

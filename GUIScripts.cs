...
public class GUIScripts : MonoBehaviour
{
	...
	public void UpdateTmpTimes()
	{
		...
		for (int i = 0; i < this.state.orbState.Length; i++)
		{
			...
			if (i % 10 == 0)
			{
				string[] array2 = array;
				int num27 = i;
				array2[num27] = "\n_" + (i / 10).ToString() + "_:" + array2[num27];
			}
		}
	...
	public void DrawRecentOrbs(float boxSize, float boxOffset)
	{
		...
		float boxWOffset = boxSize + boxOffset;
		for (int i = 0; i < this.state.orbCollectionList.Count; i++)
		{
			Vector2 xy = new Vector2((float)(i % 10), (float)((i - i % 10) / 10));
			GUI.Box(new Rect(xy.x * boxWOffset, xy.y * boxWOffset, boxSize, boxSize), new GUIContent(this.state.orbCollectionList[i].ToString("D02")));
		}
	}
	...
	public void DrawOrbsTotal(float boxSize, float boxOffset)
	{
		...
		for (int i = 0; i < this.state.orbState.Length; i++)
		{
			...
			GUI.Box(new Rect(xy.x * boxWOffset, xy.y * boxWOffset, boxSize, boxSize), content);
		}
	}
